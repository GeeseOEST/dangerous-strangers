...
class TestCoreFunctions:
    ...
    @pytest.mark.parametrize("attribute", ["archetype", "level","hitdice", "core_stat", "proficiencies", "selectable_proficiencies", "equipment", "features"]) 
...
class TestSetProficiencies:
    ...
    def test_standard_set_correctly(self, archetype_resource, area):
        try:
            assert (archetype_resource.proficiencies[area] == archetype_resource.rules["proficiencies"]["standard"][area])
        except KeyError:
            pass
    ...
    def test_selectable_set_correctly(self, archetype_resource, area):
        try:
            assert (archetype_resource.selectable_proficiencies[area] == archetype_resource.rules["proficiencies"]["selectable"][area])
        except KeyError:
            pass
...
class TestSetEquipment: # Still missing a test to be 100% certain the random item selection is working, but currently it's fairly safe IMHO given that the len is correct and format is correct
    ...
    def test_correct_amount_of_equipment(self, archetype_resource):
        assert (len(archetype_resource.equipment) == len(archetype_resource.rules["equipment"]))

    def test_equipment_correctly_set(self, archetype_resource):
        try:
            for item in range(len(archetype_resource.rules["equipment"])):
                assert (archetype_resource.equipment[item] in archetype_resource.rules["equipment"][item]), f"{archetype_resource.equipment[item]} should be in equipment[{item}]"
        except IndexError:
                assert False, f"{item} is not a valid index, equipment list may not yet exist. THIS IS AN INDEXERROR AND MAY BE SOMETHING ELSE!"
...
class TestSetFeatures:
    ...
    def test_features_match_level_simple(self, archetype_resource):
        ...
        for level in range(archetype_resource.level+1): # Range is 0 indexed and therefore since 1st level feats are marked as 1, need to do that
            correct_number_of_feats += len(archetype_resource.rules["feats"][level])
        assert (correct_number_of_feats == len(archetype_resource.features)), f"Archetype has {len(archetype_resource.features)} feats, should have {correct_number_of_feats}"
